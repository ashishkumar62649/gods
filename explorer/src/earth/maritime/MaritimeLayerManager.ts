...
export class MaritimeLayerManager {
  ...
  tickVessels() {
    ...
    for (const entry of this.entries.values()) {
      const speedKn = entry.vessel.speedKn ?? 0;
      if (speedKn <= 0.1) {
        entry.lastTickMs = now;
        continue;
      }

      const dtSec = (now - entry.lastTickMs) / 1000;
      if (dtSec <= 0) continue;
      entry.lastTickMs = now;

      const speedMps = speedKn * KNOTS_TO_MPS;
      const distance = speedMps * dtSec;
      const headingRad = ((entry.vessel.headingDeg ?? 0) * Math.PI) / 180;

      const latRad = (entry.lat * Math.PI) / 180;
      const lonRad = (entry.lon * Math.PI) / 180;
      ...
      entry.lat = (nextLatRad * 180) / Math.PI;
      entry.lon = (nextLonRad * 180) / Math.PI;
      entry.point.position = Cartesian3.fromDegrees(entry.lon, entry.lat, 120);
      moved = true;
    }
